  ...
  const handleAppointment = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post("http://localhost:4000/api/v1/appointment/post", {
        firstName, lastName, email, phone, nic, dob, gender, address, department,
      ...
      setFirstName(""), setLastName(""), setEmail(""),
        setPhone(""), setNic(""), setDob(""), setGender(""),
        setAppointmentDate(""), setDepartment(""),
        setDoctorFirstName(""), setDoctorLastName(""),
        setHasVisited(""), setAddress("");
  ...
  return (
    <>
      <div className="container form-component appointment-form">
        <h2>Appointment</h2>
        <p>Book an appointment with our experienced doctors quickly and easily. Choose your preferred department, select a convenient date, and receive quality healthcare with personalized attention.</p>
        ...
        <form onSubmit={handleAppointment}>
          ...
          <div>
            <input type="number" placeholder="NIC 13 digits" value={nic} onChange={(e) => setNic(e.target.value)} />
          ...
          <div>
            ...
            >
              <option value="">Select Doctor</option>
              {doctors.filter((doctor) => doctor.doctorDepartment === department).map((doctor, index) => (
                <option key={index} value={JSON.stringify({ firstName: doctor.firstName, lastName: doctor.lastName })}>
                  {doctor.firstName} {doctor.lastName}
                </option>
              ))}
            </select>
